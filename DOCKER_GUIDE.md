...
0. [Локальный запуск (без Docker)](#0-локальный-запуск-без-docker)
...
## 0. Локальный запуск (без Docker)

Запуск сайта локально на машине разработчика (FastAPI-бэкенд + Vite-фронтенд).

> ⚠️ Нужен доступный PostgreSQL (например, `109.94.211.122:5432`) и заполненный `.env` (переменные `db_user`, `db_password`, `db_host`, `db_port`, `db_database` и токены BCS/EXANTE).
...
- `run_local.py` запускает uvicorn с флагом `--reload` (авто-перезагрузка при изменении кода)

### 0.2. Фронтенд (Vue + Vite)

```bash
# из папки frontend
npm install       # один раз, если ещё не ставили
npm run dev
```

- Фронтенд поднимется на `http://localhost:5173` и откроется в браузере
- Vite проксирует запросы `/api/*` на `http://localhost:8000` (настройка в `frontend/vite.config.js`), так что CORS не нужен
- Запускайте бэкенд **до** фронтенда — прокси должен «дожить» до `localhost:8000`

### 0.3. Остановка

```bash
# бэкенд: Ctrl+C в его терминале
# фронтенд: Ctrl+C в его терминале (или q в терминале Vite)
```
